...
function Wallet() {
  ...
  function handleDisconnect() {
    toastConfirm("Disconnect Wallet", "", "", "320px")
      .fire()
      .then((result) => {
      ...
      <div className="row my-4 justify-content-center">
        <div className="col-9 col-md-7 col-lg-6 col-xl-5 p-0">
          <div className="d-flex rounded-3 border-1 border-gray p-2 justify-content-between">
            <div>
              <b className="text-black mx-1">Wallet Address :</b>{" "}
              <a
                target="_blank"
                href={`${ScanUrl}/address/${activeAccount?.address}`}
                className="mx-1 text-decoration-none cursor-pointer text-break"
                rel="noreferrer"
              >
                {shortenAddress(activeAccount?.address, 6)}
              </a>
              <FaCopy
                onClick={copyAddress}
