...
const SendFundsModal = ({ tokenToSend, show, onHide }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const treasurySigner = new ethers.Wallet(
      ...
      <Modal.Body>
        <Form onSubmit={handleSubmit}>
          <Form.Group className="mb-3">
            ...
            <div
              class="d-flex gap-2 align-items-center rounded-2 px-2 py-1 cursor-pointer"
            ...
            >
              ...
              <div>
                ...
                <div className="" style={{ fontSize: "12px" }}>
                  {+tokenToSend?.balance >= 0
                    ? Number(tokenToSend?.balance).toFixed(3)
                    : 0}{" "}
                  {tokenToSend?.symbol}
                </div>
              </div>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Text className="text-danger">
              {amount?.toString()?.length && +amount <= 0 ? (
                <>Please enter valid amount to send.</>
              ) : null}
            </Form.Text>
          </Form.Group>
          <Button
            disabled={+amount <= 0 || !isAddress(recipient) || sending}
